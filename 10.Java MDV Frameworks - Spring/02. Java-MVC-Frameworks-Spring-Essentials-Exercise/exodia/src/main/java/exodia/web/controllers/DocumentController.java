...
public class DocumentController {
    ...
    public ModelAndView scheduleConfirm(@ModelAttribute DocumentScheduleBindingModel model, ModelAndView modelAndView, HttpSession session) {
        ...
        if(session.getAttribute("username") == null){
            ...
            DocumentServiceModel documentServiceModel =
                    documentService.scheduleDocument(
                            modelMapper.map(
                                    model, DocumentServiceModel.class
                            )
                    );

            if (documentServiceModel == null) {
                throw new IllegalArgumentException("Document creating failed");
            }
    ...
    public ModelAndView details(@PathVariable("id") String id, ModelAndView modelAndView, HttpSession session) {
        ...
        if(session.getAttribute("username") == null){
            ...
            modelAndView.addObject("model",
                    modelMapper.map(documentServiceModel, DocumentDetailsViewModel.class));
        }
    ...
    public ModelAndView printConfirm(@PathVariable("id") String id, ModelAndView modelAndView){
        ...
        if(!documentService.printDocumentById(id)){
